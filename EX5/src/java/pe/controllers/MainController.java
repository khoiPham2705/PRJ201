...
//                your code here
   ...
   public void carList1(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException, ClassNotFoundException, SQLException {
        try{
            CarDAO cd = new CarDAO();
            List<CarDTO> list = cd.select();
            
            HttpSession session = request.getSession();
            session.setAttribute("list", list);
    } catch (Exception ex) {
            ex.printStackTrace();
            request.setAttribute("message", "Error reading car table");
        }
    ...
    protected String create_handler(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            switch(op){
                case "create":
                    ...
                    if (id == null || !id.matches("^C-\\d{3}$")) {
                        request.setAttribute("message", "ID must be C-XXX");
                        return url;
                    }
                    
                    
                    CarDAO cd = new CarDAO();
                    if (cd.isCarIDExists(id)) {  // Gọi hàm kiểm tra ID có tồn tại chưa
                request.setAttribute("message", "ID is already exist.");
                return url;
            }
                    cd.create(id,name,description,price,speed,status);
                     url = "carList.jsp";
                    break;
                case "cancel":
                    url = "carList.jsp";
            ...
            ex.printStackTrace();
            request.setAttribute("message", "Missing Information");
        }
        carList1(request, response);
        return url;
